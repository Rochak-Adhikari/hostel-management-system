...
import { ArrowRightLeft, X, FileText, HelpCircle } from "lucide-react";
import { useQuery } from "@tanstack/react-query";
import { getRoomChangeRequestsByStudent } from "@/api/roomChangeRequestApi";
...
export default function GuardianRoomRequests() {
  ...
  // linked student ko room change requests fetch garne
  const { data: requestsRes, isPending } = useQuery({
    queryKey: ["childRoomChangeRequests", currentUser?.linked_student],
    queryFn: () => getRoomChangeRequestsByStudent(currentUser.linked_student),
  ...
  if (!currentUser) {
    return (
      <div className="bg-white rounded-2xl border border-gray-200 p-8 text-center">
        <p className="text-gray-500">Please log in to view room change requests.</p>
      </div>
    );
  }
  ...
  if (isPending) {
    return (
      <div className="bg-white rounded-2xl border border-gray-200 p-8 text-center">
        <p className="text-gray-500">Loading child's room change requests...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-black text-gray-900 tracking-tight">Child's Room Change Requests</h1>
        <p className="text-sm text-gray-500 mt-0.5">
          View all room change requests filed by your linked child.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_380px] gap-5 items-start">
        ...
        <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm">
          <div className="px-5 py-4 border-b border-gray-100 bg-gray-50 flex items-center">
            ...
            <h2 className="text-sm font-semibold text-gray-700">Room Change Requests Ledger</h2>
          </div>
          ...
          <div className="divide-y divide-gray-100">
            {requests.length === 0 ? (
              <div className="p-8 text-center text-gray-400">
                ...
                <p className="text-xs">No room change requests filed by child yet.</p>
              </div>
            ) : (
              requests.map((r) => (
                <div
        ...
        {selectedRequest ? (
          <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm space-y-4">
            ...
            <div className="px-5 pb-5 space-y-4">
              {selectedRequest.preferredRoomType && (
                <div>
                  <p className="text-[10px] uppercase tracking-widest text-gray-400 font-medium">Preferred Room Type</p>
                  <p className="text-xs font-bold text-gray-800 mt-0.5">{selectedRequest.preferredRoomType}</p>
                </div>
              )}
          ...
          <div className="hidden xl:block bg-white border border-gray-200 rounded-2xl p-5 text-center text-gray-400">
            <ArrowRightLeft className="mx-auto mb-2 text-gray-300" size={32} />
