...
import { Search, Trash2, X, CalendarDays } from "lucide-react";
import { useQuery, useMutation, useQueryClient } from "@tanstack/react-query";
import { getAllLeaveRequests, updateLeaveRequest, deleteLeaveRequest } from "@/api/leaveRequestApi";
...
export default function AdminLeaveRequestsPage() {
  ...
  const [newStatus, setNewStatus] = useState<LeaveRequest["status"]>("Pending");
  const [adminNote, setAdminNote] = useState("");

  // Sabai leave requests fetch garne
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, status, adminNote }: { id: string; status: LeaveRequest["status"]; adminNote?: string }) =>
      updateLeaveRequest(id, { status, adminNote }),
    onSuccess: () => {
  ...
  function openDetail(r: LeaveRequest) {
    setSelected(r);
  ...
  return (
    <div className="space-y-8">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-bold text-black">Leave Requests</h1>
        <p className="text-gray-500 mt-1">Review, approve, or reject student leave permission applications.</p>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
          ...
          <div className="divide-y divide-gray-100">
            ...
            {filtered.length === 0 && (
              <p className="py-10 text-center text-gray-400 text-sm">No leave requests found.</p>
            )}
          </div>
        </div>
          ...
          {selected ? (
            <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
              ...
              <div className="p-5 space-y-4">
                ...
                <div>
                  <p className="text-xs text-gray-500">Leave Duration / Dates</p>
                  <p className="font-semibold text-gray-900">
                    {new Date(selected.fromDate).toLocaleDateString("en-GB")} → {new Date(selected.toDate).toLocaleDateString("en-GB")}
                  </p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <select
                    value={newStatus}
                    onChange={(e) => setNewStatus(e.target.value as LeaveRequest["status"])}
